...
serve(async (req) => {
  ...
  try {
    ...
    const recipientEmail = commData.clearing_agent?.contact_email
    if (!recipientEmail) {
      throw new Error('Recipient email not found in clearing agent.')
    }
    ...
    const attachments = commData.clearing_agent_communication_documents.map(docWrapper => {
    ...
    console.log('Sending email to clearing agent:')
    ...
    const emailPayload: any = {
      from: 'Imports 360 <onboarding@resend.dev>', // Replace with your verified domain
      to: [recipientEmail],
    ...
    return new Response(JSON.stringify({ 
      ...
      message: ccEmails.length > 0 
        ? `Email sent successfully to clearing agent with ${ccEmails.length} CC recipient(s)`
        : 'Email sent successfully to clearing agent'
    }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    })
