-- Add mode_of_transport to filters
-- This script updates the view and filter function to support mode_of_transport filtering

-- Step 1: Drop and recreate the view with mode_of_transport
...
-- Step 2: Update the filter function to include mode_of_transport parameter
...
BEGIN
  ...
  WHERE
    ...
    (p_commodity IS NULL OR p_commodity = '' OR s.commodity = p_commodity) AND
    ...
    (p_mode_of_transport IS NULL OR p_mode_of_transport = '' OR s.mode_of_transport = p_mode_of_transport);
END;
$$ LANGUAGE plpgsql;
