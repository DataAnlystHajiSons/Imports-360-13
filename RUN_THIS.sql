-- Complete fix: Drop views, update enum, recreate views
...
-- Step 2: Drop views that depend on current_stage
DROP VIEW IF EXISTS v_shipment_stage_checklist;
DROP VIEW IF EXISTS v_shipments_with_details;
DROP VIEW IF EXISTS v_shipments_with_all_details;
DROP VIEW IF EXISTS v_forecast_with_order_status;
...
SELECT
  ...
  -- LC Opening (now includes LC sharing)
...
-- Verify
SELECT 'SUCCESS! LC stages merged. Refresh your browser.' as result;
