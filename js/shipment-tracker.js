    ...
    const STAGE_DETAILS = {
        ...
        "bank_debit_advice": { name: "Bank Debit Advices", icon: "fa-solid fa-receipt", responsible: "Finance Team", duration: "1 day" },
        ...
        "good_declaration": { name: "Good Declaration", icon: "fa-solid fa-file-shield", responsible: "Logistics", duration: "2 days" },
    ...
    async function initializeTracker() {
        ...
        // Fetch all stage data in parallel to calculate incomplete fields for EVERY stage
        const stageTableNames = [...new Set(Object.values(STAGE_CONFIG).map(c => c.table))];
        const stageDataPromises = stageTableNames.map(tableName => {
            if (tableName === 'forecast') {
                return supabase.from('shipment_products')
                    .select('*, product_variety(*, forecast(*))')
                    .eq('shipment_id', shipmentId)
                    .then(res => {
                        ...
                        if (res.data) {
                            for (const product of res.data) {
                                const f = product.product_variety && product.product_variety.forecast;
                                if (f) {
                                    forecastData = Array.isArray(f) ? f[0] : f;
                                    if (forecastData) break;
                                }
                            }
                        }
        ...
        STAGE_ORDER.forEach((stageKey, index) => {
            ...
            if (config && config.fields) {
                config.fields.forEach(field => {
                    // Skip readonly fields, metadata fields, system timestamps, and URLs
                    if (
                        field.readonly || 
                        field.name === 'id' || 
                        field.name === 'shipment_id' ||
                        field.name.endsWith('_at') ||
                        field.name.endsWith('_by') ||
            ...
            // NEW LOGIC: Status is green (completed) if all fields are filled, regardless of global current_stage
            if (totalFields > 0 && incompleteCount === 0) {
            ...
            ${incompleteCount > 0 ? `<div class="incomplete-badge">${incompleteCount}</div>` : ''}
            <div class="stage-label">${stage.name}</div>
            <div class="tooltip">
                <strong>${stage.name}</strong><br>
                ${stage.responsible}<br>
                Duration: ${stage.duration}
                ${incompleteCount > 0 ? `<br><br><span style="color: #f59e0b">⚠️ ${incompleteCount} fields incomplete</span>` : ''}
            </div>
            `;

            // Add visual indicator if this stage has a target date
            if (window.stageTargetDatesManager && stageKey !== 'documents') {
                const targetDate = window.stageTargetDatesManager.getTargetDateForStage(stageKey);
                if (targetDate) {
                    stageNode.setAttribute('data-has-target', 'true');
                    stageNode.setAttribute('data-target-status', targetDate.status);
                    
                    // Update tooltip to include target date info
                    const tooltip = stageNode.querySelector('.tooltip');
                    tooltip.innerHTML += `<br><br><strong>🎯 Target:</strong> ${new Date(targetDate.target_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}<br><span style="color: ${targetDate.status === 'overdue' ? '#ef4444' : targetDate.status === 'warning' ? '#f59e0b' : '#10b981'}">${targetDate.days_remaining} days ${targetDate.days_remaining >= 0 ? 'remaining' : 'overdue'}</span>`;
                }
            }
        ...
        // Initialize Stage Target Dates Manager
        ...
        // Render target dates summary panel at the top of sidebar
        const targetSummaryHtml = window.stageTargetDatesManager.renderTargetDatesSummary();
        const sidebarElement = document.querySelector('.tracker-sidebar');
        const supplierSection = document.querySelector('.supplier-details-section');
        if (sidebarElement && supplierSection) {
            // Remove existing summary container to prevent duplication
            const existingSummary = document.getElementById('target-dates-summary-container');
            if (existingSummary) {
                existingSummary.remove();
            }
            
            const targetSummaryContainer = document.createElement('div');
            targetSummaryContainer.id = 'target-dates-summary-container';
            targetSummaryContainer.innerHTML = targetSummaryHtml;
            sidebarElement.insertBefore(targetSummaryContainer, supplierSection.nextSibling);
        }
        
        populateTimeline(currentStageIndex, stageDataMap, hasDocuments);
    ...
    async function populateTimeline(currentStageIndex) {
        ...
        // Fetch all stage data in parallel to calculate incomplete fields
        const stageTableNames = [...new Set(Object.values(STAGE_CONFIG).map(c => c.table))];
        const stageDataPromises = stageTableNames.map(tableName => {
            if (tableName === 'forecast') {
                return supabase.from('shipment_products')
        ...
        // Fetch shipment award info for special display in timeline
        const { data: awardInfo } = await supabase
        ...
        STAGE_ORDER.forEach((stageKey, index) => {
            ...
            // Special handling for award_shipment date display
            if (stageKey === 'award_shipment' && awardInfo && awardInfo.awarded_at) {
                const awardDate = new Date(awardInfo.awarded_at);
                timeText = `Awarded on ${awardDate.toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}`;
            }
            ...
            if (actualDuration && expectedDuration) {
                ...
                if (actualDuration > expectedDuration) {
                    exceeded = true;
                    // Keep the primary status class but add exceeded for styling
            ...
            // Add incomplete badge if there are missing fields
            const badgeHtml = (incompleteCount > 0 && stageKey !== 'documents') 
                ? `<span class="incomplete-badge" title="${incompleteCount} incomplete fields">${incompleteCount}</span>` 
                : '';
            ...
            if (window.stageTargetDatesManager) {
                const targetDateHtml = window.stageTargetDatesManager.renderTimelineTargetDate(stageKey);
                if (targetDateHtml) {
                    hasTargetDate = true;
                    const timelineContent = timelineItem.querySelector('.timeline-content');
                    const targetDateDiv = document.createElement('div');
                    targetDateDiv.innerHTML = targetDateHtml;
                    timelineContent.appendChild(targetDateDiv.firstChild);
                }
            }
            
            // Add "Set Target" or "Edit Target" button (except for documents stage)
            if (stageKey !== 'documents') {
                const setTargetBtn = document.createElement('button');
                setTargetBtn.className = 'set-target-btn';
                
                // Change button text and style based on whether target exists
                if (hasTargetDate) {
                    setTargetBtn.innerHTML = '<i class="fas fa-edit"></i> Edit Target';
                    setTargetBtn.classList.add('has-target');
                } else {
                    setTargetBtn.innerHTML = '<i class="fas fa-calendar-plus"></i> Set Target';
                }
                
                setTargetBtn.onclick = (e) => {
                    e.stopPropagation();
                    if (window.stageTargetDatesManager) {
                        window.stageTargetDatesManager.openTargetDateModal(stageKey, stage.name);
                    }
                };
                timelineItem.appendChild(setTargetBtn);
            }
    ...
    async function openStageModal(stageName) {
        ...
        // All accessibility restrictions removed - any stage can be opened
    ...
    async function renderStageView() {
        ...
        if (currentStageData.stageName === 'freight_query') {
            ...
            error = queryError;
        } else if (config.table === 'forecast') {
            // Forecast is linked via product_variety_id, not shipment_id
            const { data: resData, error: resError } = await supabase
                .from('shipment_products')
                .select(`product_variety(forecast(${selectString}))`)
                .eq('shipment_id', shipmentId)
                .limit(1);

            data = resData && resData.length > 0 && resData[0].product_variety && resData[0].product_variety.forecast ? resData[0].product_variety.forecast[0] : null;
            error = resError;
        } else {
            const { data: queryData, error: queryError } = await supabase
                .from(config.table)
        ...
        if (currentStageData.stageName === 'ip_number') {
            ...
            for (const field of currentStageData.config.fields) {
                ...
                if ((field.type === 'datetime-local' || field.type === 'date') && value && value !== 'N/A') {
    ...
    async function saveStageDetails() {
        ...
        try {
            ...
            let query;
            if (stageName === 'forecast') {
                delete updates.shipment_id;
                let productVarietyId = currentStageData.details.product_variety_id;
                ...
                if (!productVarietyId) {
                    ...
                    if (spData && spData.length > 0) {
                        productVarietyId = spData[0].product_variety_id;
                    }
                }

                if (productVarietyId) {
                    updates.product_variety_id = productVarietyId;
                    ...
                    throw new Error("Could not determine product_variety_id for forecast");
                }
            } else if (stageName === 'freight_query') {
                if (currentStageData.details && currentStageData.details.id) {
                    query = supabase.from(config.table).update(updates).eq('id', currentStageData.details.id);
                } else {
                    query = supabase.from(config.table).insert(updates);
...
Direct Data Count: ${directData?.length || 0}`);
    ...
    // Global refresh function for stage target dates
    window.refreshStageTargetDates = async function() {
        if (window.stageTargetDatesManager) {
            await window.stageTargetDatesManager.loadTargetDates();
            await initializeTracker(); // Refresh the entire tracker
        }
    };
    ...
    if (shipmentId) {
        ...
        document.body.innerHTML = '<p class="error-message">No shipment ID provided</p>';
    }
