...
const formatPKR = (value) => {
    return new Intl.NumberFormat('en-PK', { style: 'currency', currency: 'PKR', minimumFractionDigits: 2 }).format(value);
};
...
function updateKpis(data) {
    ...
    const kpis = data.reduce((acc, payment) => {
        const remaining = payment.total_amount - payment.amount_paid;
        const rate = payment.exchange_rate || payment.dollar_rate || 1; // Fallback for transition
        const remainingPKR = remaining * rate;
        const amountPaidPKR = (payment.amount_paid || 0) * rate;

        const isPaid = payment.status === 'paid';
        const dueDate = payment.due_date ? new Date(payment.due_date) : null;

        if (!isPaid) {
            acc.totalPayable += remainingPKR;

            if (dueDate && dueDate < now && payment.status !== 'partially_paid') {
                acc.totalOverdue += remainingPKR;
            }

            if (dueDate && dueDate.getMonth() === currentMonth && dueDate.getFullYear() === currentYear) {
                acc.payableThisMonth += remainingPKR;
            }
        }

        const paymentDate = new Date(payment.updated_at);
        if (payment.amount_paid > 0 && paymentDate.getFullYear() === currentYear) {
            acc.paymentsMadeYtd += amountPaidPKR;
        }

        return acc;
    }, { totalPayable: 0, totalOverdue: 0, payableThisMonth: 0, paymentsMadeYtd: 0 });

    totalPayableEl.textContent = formatPKR(kpis.totalPayable);
    totalOverdueEl.textContent = formatPKR(kpis.totalOverdue);
    payableThisMonthEl.textContent = formatPKR(kpis.payableThisMonth);
    paymentsMadeYtdEl.textContent = formatPKR(kpis.paymentsMadeYtd);
}
...
function renderTable(data) {
    ...
    paginatedData.forEach(p => {
        ...
        const rate = p.exchange_rate || p.dollar_rate;
        const rateDisplay = rate ? `Rs ${parseFloat(rate).toFixed(2)}` : 'N/A';

        tr.innerHTML = `
            <td>${p.supplier.name}</td>
            <td>${p.shipment.reference_code}</td>
            <td>${bankName}</td>
            <td>${formatCurrency(p.total_amount, currency)}</td>
            <td>${formatCurrency(p.amount_paid, currency)}</td>
...
export function initPaymentSection(supabase, shipmentData, totalAmount) {
    ...
    const rate = payment.exchange_rate || payment.dollar_rate;
    ...
    let paymentHtml = `
        ...
        <div class="summary-grid">
            ...
            <div class="summary-item">
                ...
                <span class="value">${formatCurrency(payment.total_amount, currency)}</span>
            </div>
            <div class="summary-item">
                <span class="label">Amount Paid</span>
                <span class="value">${formatCurrency(payment.amount_paid, currency)}</span>
            </div>
            <div class="summary-item">
                <span class="label">Amount Remaining</span>
                <span class="value">${formatCurrency(amountRemaining, currency)}</span>
            </div>
            <div class="summary-item">
                <span class="label">Exchange Rate</span>
                <span class="value">${rate ? 'Rs ' + parseFloat(rate).toFixed(2) : 'N/A'}</span>
            </div>
            <div class="summary-item">
...
async function openPaymentLogModal(payment) {
    ...
    summaryContainer.innerHTML = `
        <div class="summary-item">
            <h4>Total Amount</h4>
            <p>${formatCurrency(payment.total_amount, currency)}</p>
        </div>
        <div class="summary-item">
            <h4>Amount Paid</h4>
            <p>${formatCurrency(payment.amount_paid, currency)}</p>
        </div>
        <div class="summary-item remaining">
            <h4>Remaining</h4>
            <p>${formatCurrency(amountRemaining, currency)}</p>
        </div>
        <div class="summary-item">
            <h4>Exchange Rate</h4>
            <p>${rate ? 'Rs ' + parseFloat(rate).toFixed(2) : 'N/A'}</p>
        </div>
...
async function loadTransactions(supplierPaymentId) {
    // We need to get the currency for this payment
        ...
        html += `
            <div class="transaction-item ${methodClass}">
                ...
                <div class="transaction-details">
                    <span class="amount">${formatCurrency(tx.amount, currency)}</span>
                ...
                <div class="transaction-meta">
                    ...
                    ${tx.exchange_rate || tx.dollar_rate ? `<span style="margin-left: 10px; color: var(--text-secondary-color);">@ Rs ${parseFloat(tx.exchange_rate || tx.dollar_rate).toFixed(2)}</span>` : ''}
...
async function saveNewTransaction() {
    ...
    const exchangeRate = document.getElementById('payment-dollar-rate').value;
