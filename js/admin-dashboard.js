...
function getStagebadge(stage) {
  ...
  return `<div class="stage-badge" style="background-color: ${config.color}15; border-color: ${config.color}; color: ${config.color}" title="${config.label}">
...
async function loadShipments(searchTerm = '', filters = {}) {
  ...
  data.forEach(s => {
    ...
    const progressBarHtml = `
      <div class="progress-bar-container" style="width: 100%; max-width: 150px;">
        <div class="progress-header" style="display: flex; justify-content: space-between; font-size: 12px; margin-bottom: 4px;">
          <span style="color: var(--text-secondary-color, #64748b);">${completedCount}/${totalCount}</span>
          <span style="font-weight: 600; color: #3b82f6;">${progressPercent}%</span>
        </div>
        <div class="progress-bar" style="height: 6px; background-color: #e2e8f0; border-radius: 3px; overflow: hidden;">
          <div class="progress-fill" style="width: ${progressPercent}%; height: 100%; background: linear-gradient(90deg, #3b82f6, #93c5fd); border-radius: 3px; transition: width 0.3s ease;"></div>
        </div>
      </div>
    `;
    ...
    html += `<tr class="enhanced-row" data-shipment-id="${s.id}" data-status="${s.status}">
      ...
      <td class="stage-cell">
        <div class="cell-content" title="Latest Activity">
          ${stageBadge}
        </div>
      </td>
