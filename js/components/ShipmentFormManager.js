...
export class ShipmentFormManager {
  ...
  setupCurrencyLogic() {
    const currencySelect = this.form.querySelector('[name="currency"]');
    const exchangeRateLabel = document.getElementById('exchange-rate-label');
    const freightChargesLabel = document.getElementById('freight-charges-label');
    ...
    currencySelect.addEventListener('change', () => {
      ...
      if (exchangeRateLabel) {
        exchangeRateLabel.innerHTML = `Exchange Rate (PKR/${currency}): <span class="required">*</span>`;
      }
      if (freightChargesLabel) {
        freightChargesLabel.innerHTML = `Freight Charges (${currency}): <span class="required">*</span>`;
      }
    });
  }
  ...
  populateDetailsReview() {
    ...
    const referenceCode = this.form.querySelector('[name="reference_code"]')?.value;
    const type = this.form.querySelector('[name="type"]').value;
    const modeOfTransport = this.form.querySelector('[name="mode_of_transport"]').value;
    const incoTerm = this.form.querySelector('[name="inco_term"]').value;
    const freightCharges = this.form.querySelector('[name="freight_charges"]')?.value;
    ...
    let html = '';
    html += `<div class="review-item"><span class="review-item-label">Reference Number:</span><span class="review-item-value"><strong>${referenceCode || 'Not provided'}</strong></span></div>`;
  ...
  async handleSubmit(e) {
    ...
    try {
      ...
      if (checkError) {
        throw new Error(`Error checking reference code: ${checkError.message}`);
      }
      
      if (existing) {
        this.showMessage(`Reference code "${formData.reference_code}" already exists. Please use a unique reference number.`, 'error');
        return;
      }
      ...
      // Use the manually entered reference code (no auto-generation)
  ...
  getFormData() {
    ...
    const currencyValue = this.form.querySelector('[name="currency"]')?.value;
    const exchangeRateValue = this.form.querySelector('[name="exchange_rate"]')?.value;
    ...
    return {
      ...
      exchange_rate: exchangeRateValue ? parseFloat(exchangeRateValue) : null,
  ...
  toggleFreightChargesField() {
    ...
    // Enable for FOB, CFR, and CPT
    if (['FOB', 'CFR', 'CPT'].includes(incotermValue)) {
