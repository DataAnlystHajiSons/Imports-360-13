...
SELECT
    ...
    (
        -- Calculate overall progress based on data presence across tables
        -- This is a simple count of completed major milestones
        ...
        (CASE WHEN EXISTS (SELECT 1 FROM public.insurance WHERE shipment_id = s.id) THEN 1 ELSE 0 END) +
        (CASE WHEN EXISTS (SELECT 1 FROM public.docs_to_clearing_agent WHERE shipment_id = s.id) THEN 1 ELSE 0 END) +
        (CASE WHEN EXISTS (SELECT 1 FROM public.clearing_agent_bill WHERE shipment_id = s.id) THEN 1 ELSE 0 END) +
        (CASE WHEN EXISTS (SELECT 1 FROM public.bility WHERE shipment_id = s.id) THEN 1 ELSE 0 END) +
    ...
    10 AS total_milestones_count -- Total number of major milestone tables checked above
FROM
    public.shipment s
LEFT JOIN
