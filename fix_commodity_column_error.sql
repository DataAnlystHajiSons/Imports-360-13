-- ============================================================================
-- FIX: Commodity Column Error - pv.commodity does not exist
-- ============================================================================
-- The error "column pv.commodity does not exist" occurs because:
-- 1. product_variety table has commodity_id (foreign key)
-- 2. commodity table has id and name columns
-- 3. Need to JOIN tables to check commodity name
...
BEGIN
  ...
  CASE p_to_stage
    ...
    -- ========================================================================
    -- FIXED ORDER: PROFORMA comes BEFORE PURCHASE ORDER
    -- ========================================================================
    WHEN 'proforma' THEN
      -- Proforma depends on availability_confirmation
      RETURN EXISTS (
    ...
    WHEN 'purchase_order' THEN
      -- Purchase order depends on proforma
      RETURN EXISTS (
    ...
    -- ========================================================================
    WHEN 'invoice' THEN
      -- Invoice depends on purchase_order
      RETURN EXISTS (
...
-- Also update the stage_edge table for the new proforma/purchase_order order
-- Remove old edges
...
-- Test query to verify the commodity join works
SELECT 'Testing commodity join:' as info;
SELECT 
  sp.shipment_id,
  pv.product_name,
  pv.variety_name,  
