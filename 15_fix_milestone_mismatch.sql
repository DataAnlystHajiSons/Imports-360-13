-- 15_fix_milestone_mismatch.sql
-- This migration updates the shipment_awarded check inside v_shipments_with_all_details to not require freight_quote_response_id or notes, aligning with frontend validation.

BEGIN;

-- 1. Recreate the dashboard view to include the new alignment checks
...
SELECT
    ...
    (
        ...
        (CASE WHEN EXISTS (SELECT 1 FROM public.shipment_awarded t WHERE t.shipment_id = s.id AND t.awarded IS NOT NULL) THEN 1 ELSE 0 END) +
...
-- 2. Recreate the public.filter_shipments function that was dropped by CASCADE
