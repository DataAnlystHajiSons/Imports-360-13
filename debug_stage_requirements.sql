...
BEGIN
  ...
  AND pv.commodity_id = (SELECT id FROM commodity WHERE name = 'Seed')
  LIMIT 1;
  ...
  CASE p_to_stage
    ...
    WHEN 'original_docs' THEN
      RAISE NOTICE 'DEBUG STAGE: Checking requirements for original_docs for shipment %', p_shipment_id;
      DECLARE
        v_file_url text;
      BEGIN
        SELECT nnd.file_url INTO v_file_url FROM public.non_negotiable_docs nnd WHERE nnd.shipment_id = p_shipment_id;
        RAISE NOTICE 'DEBUG STAGE: file_url is: %', v_file_url;
        IF v_file_url IS NOT NULL THEN
          RAISE NOTICE 'DEBUG STAGE: file_url is NOT NULL, returning TRUE';
          RETURN TRUE;
        ELSE
          RAISE NOTICE 'DEBUG STAGE: file_url is NULL, returning FALSE';
          RETURN FALSE;
        END IF;
      END;
