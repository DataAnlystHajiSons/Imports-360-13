-- Final fix: Use CASCADE to drop old enum type

BEGIN;

-- Step 1: Update shipment table
UPDATE shipment
SET current_stage = 'lc_opening'
WHERE current_stage = 'lc_shared_with_supplier';

-- Step 2: Update audit_log table (both columns)
UPDATE audit_log
SET from_stage = 'lc_opening'
WHERE from_stage = 'lc_shared_with_supplier';

UPDATE audit_log
SET to_stage = 'lc_opening'
WHERE to_stage = 'lc_shared_with_supplier';

-- Step 3: Update stage_edge table
UPDATE stage_edge
SET from_stage = 'lc_opening'
WHERE from_stage = 'lc_shared_with_supplier';
...
-- Step 4: DELETE the duplicate from stage_details
DELETE FROM stage_details
WHERE stage_name = 'lc_shared_with_supplier';

-- Step 5: Drop views with CASCADE
...
-- Step 10: Drop old enum with CASCADE (this will drop and recreate dependent functions)
DROP TYPE stage_old CASCADE;

-- Step 11: Recreate view
...
SELECT 'DONE! Refresh browser (Ctrl+F5). You may need to recreate some functions.' as result;
